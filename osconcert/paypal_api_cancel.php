...
if (!$FSESSION->is_registered('customer_id')) { 
				...
				exit('.'.MODULE_PAYMENT_PAYPAL_API_CANX_FAILED);
} 
...
if (!$FSESSION->is_registered('paypal_api_token')) {
...
if (sizeof($id_query) < 1) {exit('.....'.MODULE_PAYMENT_PAYPAL_API_CANX_FAILED);}

while($id_result= tep_db_fetch_array($id_query)){
 
 //check the order_id from the previous check matches this one - if not exit 
	if ($token  != $_SESSION['paypal_api_token'] ){$FSESSION->remove('paypal_api_token');exit('Token error');}
	if ($id_result['orders_id'] != $_SESSION['paypal_api_id'] ){$FSESSION->remove('paypal_api_id');exit('Order error');} 
	tep_db_query("update orders set payment_return1 = '0', payment_return2 ='0' where payment_return2 ='".$token."'");

}

if (!strstr($_SESSION['paypal_api_token'],$token)){
                exit(MODULE_PAYMENT_PAYPAL_API_CANX_FAILED);
}
...
repopulate_cart($this_order_id,'','','Cancelled at PayPal by customer',$void);
	...
	function repopulate_cart($order_id,$redirect_page=FILENAME_CHECKOUT_PAYMENT,$extras='', $note='',$void='')
{
    ...
    if (tep_not_null($order_id)) {
        ...
        while ($order = tep_db_fetch_array($order_query)) {
			...
			//tep_db_query("update orders_products set orders_products_status = ". MODULE_PAYMENT_PAYPAL_API_CANX_ORDER_STATUS_ID . " where products_id = '" . (int)$order['products_id'] . "'");
			
			tep_db_query("update orders_products set orders_products_status = ". MODULE_PAYMENT_PAYPAL_API_CANX_ORDER_STATUS_ID . " where products_id = '" . (int)$order['products_id'] . "' and orders_id = '" . $order_id . "'");
			
			if (MODULE_PAYMENT_PAYPAL_API_DEBUG == 'True' && tep_not_null(MODULE_PAYMENT_PAYPAL_API_DEBUG_EMAIL)){
	             tep_mail('Debug', MODULE_PAYMENT_PAYPAL_API_DEBUG_EMAIL, 'PayPal cancel', "Order #: ".$order_id. " cancelled by customer (".STORE_NAME.")", STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);	
			}
