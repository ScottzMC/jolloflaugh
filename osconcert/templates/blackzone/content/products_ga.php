...
<?php
		...
		p.product_type,
		p.master_quantity 
		from " . TABLE_PRODUCTS . " p join ".TABLE_PRODUCTS_TO_CATEGORIES . " p2c on p2c.products_id=p.products_id join ".TABLE_PRODUCTS_DESCRIPTION . " pd on pd.products_id=p.products_id where p.product_type !='P' and p.product_type !='L' and p.product_type !='Q' and p.products_id >'0' " . $expires . " " . $soldout . " and p2c.categories_id = '" . $cPath . "' and pd.language_id = '" . (int)$FSESSION->languages_id . "' ". $check_restrict_customer." order by p.products_sort_order,p.products_id asc");
	...
	<div id="ga_listing" class="container  mt-5 pb-2 mb-3">
	      ...
	      <div class="row p-3 text-light bg-dark">  
        <div class="col-md-6">
...
<?php 
			...
			while ($listing = tep_db_fetch_array($listing_query)) 
		{
			...
			if (($status == 0) or ($category_ga['categories_GA'] > 0) && ($category_ga['categories_quantity_remaining'] < 1)) 
			{
				$plink = '#';
			}
			else 
	...
	<div class="col-md-4">
	  <h4><?php echo '<a href="' . $plink . '"><strong>' .  $pname . '</strong></a>';?>
			<?php 
			if(HIDE_GA_QTY=="no")
			{
	...
	<div class="col-md-3">
	  ...
	  if(($listing['products_status']<1) or ($listing['products_quantity']<1)or ($category_ga['categories_GA'] > 0) && ($category_ga['categories_quantity_remaining'] < 1))
		...
		//if (($listing['products_status']<1) or ($listing['products_quantity']<1))
