...
define('MODULE_NAVBAR_SHOPPING_CART_HAS_CONTENTS', '<span>%s ' . ITEMS . ' %s</span>');
...
?>
		<li class="dropdown">
		<a class="dropdown-toggle" href="shopping_cart.php" id="navDropdownCart" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		<?php echo sprintf(MODULE_NAVBAR_SHOPPING_CART_CONTENTS, $cart->count_contents_gt()); ?>
		</a>
		<ul>
		<li>
        <?php 
		echo '<a class="dropdown-item" href="shopping_cart.php">' . sprintf(MODULE_NAVBAR_SHOPPING_CART_HAS_CONTENTS, $cart->count_contents_gt(), $currencies->format($cart->show_total())) . '</a>';
		...
		echo '</div>' . PHP_EOL; 
		echo '<div class="dropdown-divider"></div>' . PHP_EOL;  
		echo '<a class="dropdown-item" href="checkout_shipping.php">' . MODULE_NAVBAR_SHOPPING_CART_CHECKOUT . '</a>';
		?>
		</li>
		</ul>
