...
class stripesca
{
    ...
    function install()
    {
		...
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Failed Order Status', 'MODULE_PAYMENT_STRIPESCA_CANX_ORDER_STATUS_ID', '".$status_id5."', 'Set the status of failed/cancelled orders made with this payment module to this value', '6', '40', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");

		//pending orders
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Completed Order Status', 'MODULE_PAYMENT_STRIPESCA_COMP_ORDER_STATUS_ID', '3', 'Set the status of succcesful orders made with this payment module to this value', '6', '40', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
	...
	function repopulate_cart($order_id,$redirect_page=FILENAME_CHECKOUT_PAYMENT,$extras='', $note='')
{
    ...
    if (tep_not_null($order_id)) {
        ...
        $cust_query = tep_db_query("select customers_name,  orders_status from " . TABLE_ORDERS . " where orders_id = '" . (int)$order_id . "'");
        if (tep_db_num_rows($cust_query) ) {
            $cust_query_result = tep_db_fetch_array($cust_query);
            $cust_name = $cust_query_result['customers_name'];
			$status_orig = $cust_query_result['orders_status'];
        }
	if ($status_orig != MODULE_PAYMENT_STRIPESCA_CANX_ORDER_STATUS_ID){
        ...
        tep_db_perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array);
			...
			if ($FSESSION->is_registered('stripe_cart')) {//customer only not the webhook
