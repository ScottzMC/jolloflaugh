	...
	while ($purge_results = tep_db_fetch_array($purge_query)){
		...
		function  tep_renderSeatplanCSS($cPath)
		{
			...
			while ($seat = tep_db_fetch_array($result)) 
			{
				...
				if ($seat['products_s'] >1){
				if(tep_not_null($seat['products_s'])){$trans_css .= "transform:scale(".$s.");";}	
				}
				$trans_css .= '';
			...
			{
				// draggable				
				$x=get_percentage(238,$seat['products_x']);
				$y=get_percentage(238,$seat['products_y']);
				
				if ($seat['products_w'] >0){
				$w=get_percentage(238,$seat['products_w']);
				}
				if ($seat['products_h'] >0){
				$h=get_percentage(238,$seat['products_h']);
				}
				if ($seat['products_h'] >0){
				$f=get_percentage(400,$seat['products_h']);
				}
				$r=$seat['products_r'];
				$s=$seat['products_s'];
				$indiv_css = '';
				...
				if($bgh>0)
				{
				$bg_height=get_percentage(238,$bgh);
				//$bg_height=get_percentage(325,$bgh);
				echo 'div.seatplan{height:'.$bg_height.'px;}' . "\n";
				}
				echo '</style>';
		}
		...
		function  tep_renderSeatplan($cPath)
		{
			...
			if(HAS_STAGE=='true')
			{
			$hasStage   = '<div id="stage_label">' .  $STAGE . '</div>';
			}
				
			$html   = '<!-- start: AJAX Seatplan -->' . "\n" . ''.$hasStage.'<div id="sp' . $cPath . '" class="seatplan">' . "\n";
			...
			while ($seat = tep_db_fetch_array($result)) 
			{
				...
				switch ($seat['products_status']) 
				{
					...
					//cancel fallthrough for box office:
						...
						//$wc='<i class=\'fa fa-wheelchair fa-2x\'></i>';
					//case 0:
					/* sold seats - falltrough */
					case 1:
						// available seats 
						$html .= '<li class="s ' . $this->color2class($seat['color_code']) . '" id="s' . $seat['products_id'] . '" data-rel="'.$products_original_price.'" title="' . $seat['products_name'] . " - " . $titlePrice . '">' . $seat['products_number'] . "</li>";						
						break;
					case 2:
						/* old blank seats */
						//$html .= "<li class='b'></li>";
						$html .= "<li class='b'><i class='fa fa-wheelchair fa-2'></i></li>";
						break;
					case 3:
						/* social distancing? */
		...
		function  tep_renderSeatplanDesign($cPath)
		{
			...
			{
			echo '<style>div.seatplan {background: url('.DIR_WS_TEMPLATE_IMAGES.'/grid_50.png) repeat;}</style>' . "\n";
			//echo '<style>div.seatplan {background: url('.DIR_WS_TEMPLATE_IMAGES.'/grid_100.png) repeat;}</style>' . "\n";
			//echo '<style>div.seatplan {background: url('.DIR_WS_TEMPLATE_IMAGES.'/grid_20.png) repeat;}</style>' . "\n";
