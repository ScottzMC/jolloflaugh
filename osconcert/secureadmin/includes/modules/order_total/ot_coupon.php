...
defined('_FEXEC') or die();
  ...
  function process() {
    global $PHP_SELF, $order, $currencies,$FSESSION;
	 ...
	 if (strtolower($this->calculate_tax) != 'none') $tod_amount = $this->calculate_tax_deduction($order_total, $this->deduction, $this->calculate_tax);
  ...
  function credit_selection() {
      global $FSESSION, $order, $currencies;
		...
		//get next field number
				...
				$selection_string .= '  <td width="20" >' .  tep_draw_separator('pixel_trans.gif', '10', '1') .'</td>';
     ...
     //   $selection_string .= '<tr id="show_r">' . "\n";
    //    $selection_string .= '  <td width="20" >' .  tep_draw_separator('pixel_trans.gif', '10', '1') .'</td>';
    //    $selection_string .= '  <td class="main">' .tep_template_image_button_redeem('button_redeem.gif',IMAGE_REDEEM_VOUCHER,'name="submit_redeem"'). "\n";
...
//        if (!$FSESSION->is_registered('cc_id')) tep_session_register('cc_id');
	...
	else if($FREQUEST->postvalue('coupon')!='' && $FREQUEST->postvalue('gv_redeem_code')=="") {
          tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(ERROR_NO_INVALID_REDEEM_COUPON), 'SSL'));
	}	
	else{
...
// attributes price
      if (isset($cart->contents[$product_id]['attributes'])) {
        ...
        while (list($option, $value) = each($cart->contents[$product_id]['attributes'])) {
          $attribute_price_query = tep_db_query("select options_values_price, price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . tep_db_input($prid) . "' and options_id = '" . tep_db_input($option) . "' and options_values_id = '" . tep_db_input($value) . "'");
