...
// $salt needs some thought.
  ...
  function create_coupon_code($salt="secret", $length=SECURITY_CODE_LENGTH) {
    ...
    while ($good_result == 0) {
      ...
      $query = tep_db_query("select coupon_code from " . TABLE_COUPONS . " where coupon_code = '" . (int)$id1 . "'");    
...
// Update the Customers GV account
  function tep_gv_account_update($customer_id, $gv_id) {
    ...
    if (tep_db_num_rows($customer_gv_query) > 0) {
      ...
      $gv_query = tep_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . tep_db_input($new_gv_amount) . "'");
    } else {
      $gv_query = tep_db_query("insert into " . TABLE_COUPON_GV_CUSTOMER . " (customer_id, amount) values ('" . tep_db_input($customer_id) . "', '" . $coupon_gv['coupon_amount'] . "')");
    }
  }
////
