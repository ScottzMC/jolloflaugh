...
function arrow_over(e){
	...
	if (browser.isIE){
		e.srcElement.popLeft=e.screenX;
		e.srcElement.popTop=e.screenY;
		control_name=e.srcElement.id.substr(6);
		calMgr.swapImg(control_name,'.imgOver',false);
		if (e.srcElement.rolloverClick) eval(control_name+".show()");
	} else {
		e.target.popLeft=e.pageX;
		e.target.popTop=e.pageY;
		control_name=e.target.id.substr(6);
		calMgr.swapImg(control_name,'.imgOver',false);
		if (e.target.rolloverClick) {
			alert("inner");
			eval(control_name+".show()");
		}
	}
}
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	function makeCalendar(intWhatMonth,intWhatYear,bViewOnly) {
		...
		if (popupMode!="window")
		   objectname=this.varName;
		else
		    objectname="window.opener."+this.varName;
		...
		if (!(bViewOnly)) {
			strOutput += '<td class="cal-HeadCell" align="center" width="100%"><a href="javascript:'+objectname+'.clearDay();window.close();"><img name="calbtn1" src="'+strDefBtnImgPath+'btn_del_small.gif" border="0" width="12" height="10"></a>&nbsp;&nbsp;<a href="javascript:'+objectname+'.scrollMonth(-1);" class="cal-DayLink">&lt;</a>&nbsp;<SELECT class="cal-ComboBox" NAME="cboMonth" onChange="'+objectname+'.changeMonth();">';
			...
			strOutput += '<\/SELECT><SELECT class="cal-ComboBox" NAME="cboYear" onChange="'+objectname+'.changeYear();">';
			...
			strOutput += '<\/SELECT>&nbsp;<a href="javascript:'+objectname+'.scrollMonth(1);" class="cal-DayLink">&gt;</a>&nbsp;&nbsp;<a href="javascript:'+objectname+'.hide();window.close();"><img name="calbtn2" src="'+strDefBtnImgPath+'btn_close_small.gif" border="0" width="12" height="10"></a><\/td><\/tr><tr><td width="100%" align="center">';
		}
		else {
	...
	function writeControl() {
		...
		if ((this.focusClick==false) || (this.focusClick==true && this.hideButton==false))  {
			...
			strHold+='<img align="absmiddle" border="0" name="' + this.btnName + '" src="' + strBtnImg +'" width="'+ strBtnW +'" height="'+ strBtnH +'" ' + ' id="arrow_' + this.varName+'" popLeft=100 popTop=100 ';
			
			if (this.rolloverClick==true){
				strHold+=' rolloverClick=1';
			}
			strHold+='></a>';
		}
		document.write(strHold);
		if ((this.focusClick==false) || (this.focusClick==true && this.hideButton==false))  {
			strHold='<script language="javascript">';
			strHold+='document.getElementById("arrow_' +this.varName+'").onmouseover=arrow_over;';
			strHold+='document.getElementById("arrow_' +this.varName+'").onclick=arrow_click;';
			strHold+='</script>';
			
			document.write(strHold);
		}
		//document.write('<iframe id="iframebg" src="javascript:false;" scrolling="no" frameborder="0" style="position:absolute; top:0px; left:0px; display:none; background-color:black;"></iframe>');
	}
...
// put these next 2 lines in when the tiny cal btns seem to randomly disappear
			...
			if (this.getSelectedDate()==''){
				if (!(dteCur)) {
					domlay('spiffycalendar',1,scX,scY,this.makeCalendar(dteToday.getMonth(),dteToday.getFullYear()),this);
				}
				else {
					domlay('spiffycalendar',1,scX,scY,this.makeCalendar(dteCur.getMonth(),dteCur.getFullYear()),this);
				}
			}
			else {
				if (calMgr.isDate(quote(this.getSelectedDate()),quote(this.dateFormat))) {
	...
	function hide() {
		...
		domlay('spiffycalendar',0,scX,scY,"",this);
		this.visible = false;
		calMgr.swapImg(this,'.imgUp',false);
		...
		if (scIE) {
		   if (popupMode!="window")
			dteCur.setMonth(document.forms["spiffyCal"].cboMonth.options[document.forms["spiffyCal"].cboMonth.selectedIndex].value);
		        ...
		        dteCur.setMonth(popupWindow.document.forms["spiffyCal"].cboMonth.options[popupWindow.document.forms["spiffyCal"].cboMonth.selectedIndex].value);
			domlay('spiffycalendar',1,scX,scY,this.makeCalendar(dteCur.getMonth(),dteCur.getFullYear()),this);
		}
		else if (scNN) {
		    if (popupMode!="window")
			dteCur.setMonth(document.spiffycalendar.document.forms["spiffyCal"].cboMonth.options[document.spiffycalendar.document.forms["spiffyCal"].cboMonth.selectedIndex].value);
		    	...
		    	dteCur.setMonth(popupWindow.document.spiffycalendar.document.forms["spiffyCal"].cboMonth.options[popupWindow.document.spiffycalendar.document.forms["spiffyCal"].cboMonth.selectedIndex].value);
			domlay('spiffycalendar',1,scX,scY,this.makeCalendar(dteCur.getMonth(),dteCur.getFullYear()),this);
		}
	}
		...
		if (scIE) {
		   if (popupMode!="window")
			dteCur.setFullYear(document.forms["spiffyCal"].cboYear.options[document.forms["spiffyCal"].cboYear.selectedIndex].value);
		        ...
		        dteCur.setFullYear(popupWindow.document.forms["spiffyCal"].cboYear.options[popupWindow.document.forms["spiffyCal"].cboYear.selectedIndex].value); 
			domlay('spiffycalendar',1,scX,scY,this.makeCalendar(dteCur.getMonth(),dteCur.getFullYear()),this);
		}
		else if (scNN) {
		   if (popupMode!="window")
			dteCur.setFullYear(document.spiffycalendar.document.forms["spiffyCal"].cboYear.options[document.spiffycalendar.document.forms["spiffyCal"].cboYear.selectedIndex].value);
		   	...
		   	dteCur.setFullYear(popupWindow.document.spiffycalendar.document.forms["spiffyCal"].cboYear.options[popupWindow.document.spiffycalendar.document.forms["spiffyCal"].cboYear.selectedIndex].value);
		...
		if (!(this.useDateRange)) {
			if (blnIsGreyDate) {
				...
				if (isDayToday(intLinkDay)) {
					if (!(bViewOnly)) {
						templink='<td align="center" class="'+tempClass+'">' + '<a class="cal-TodayLink" '+holTxt+' onmouseover="self.status=\' \';return true" href="javascript:'+objectname+'.changeDay(' + intLinkDay + ');window.close();">' + intLinkDay + '<\/a><\/td>';
					}
					else {
					...
					if (!(bViewOnly)) {
						templink='<td align="center" class="'+tempClass+'">' + '<a class="cal-DayLink" '+holTxt+' onmouseover="self.status=\' \';return true" href="javascript:'+objectname+'.changeDay(' + intLinkDay + ');window.close();">' + intLinkDay + '<\/a>' +'<\/td>';
					}
					else {
...
function domlay(id,trigger,lax,lay,content,objname) {
	/*
	...
	// Layer hidden
		...
		if (popupMode=="window"){
		   if (typeof popupWindow=="object" && autoclose==false){
			popupWindow.close();
			popupWindow="";
		   }
		   autoclose==false;
	...
	if (popupMode=="window"){
	 if (popupWindow=="" || popupWindow.closed==true){
	 	tempElement=document.getElementById("arrow_"+objname.varName);
		if (tempElement){
			lleft=tempElement.popLeft;
			ltop=tempElement.popTop;
		} else {
			lleft=300;
			ltop=200;
		}
		popupWindow=window.open("","test","toolbar=no,location=no,status=no,menubar=no,scollbars=no,width=180,height=150,left="+lleft+",top="+ltop);
		//alert(this.popupWindow.window.document.body.innerHTML);
		popupWindow.document.write('<html><head><title>Select Date</title></head><link rel="stylesheet" type="text/css" href="includes/javascript/spiffyCal/spiffyCal_v2_1.css">');
		popupWindow.document.write('<script language=javascript>function closeVars(){window.opener.autoclose=true;window.opener.'+objname.varName+'.hide();}</script>');
		popupWindow.document.write('<body onbeforeunload="closeVars()" marginwidth="0" marginheight="0" topmargin="0" bottommargin="0" leftmargin="0" rightmargin="0"></body></html>');
	 }
	 ...
	 popupWindow.document.body.innerHTML=content;
		...
		//mytoggleDisplay('spiffycalendar',trigger);
}

function mytoggleDisplay(div,trigger)
{
	var t=0;
	var l=0;
	var state=false;
	...
	var IfrRef = document.getElementById('iframebg');
	...
	if (trigger==0) {
   	...
   	if(state)
   	{
			...
			IfrRef.style.zIndex = -1*DivRef.style.zIndex;
