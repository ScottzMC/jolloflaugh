    ...
    function __construct() {
      ...
      if (defined('MODULE_ORDER_TOTAL_INSTALLED') && tep_not_null(MODULE_ORDER_TOTAL_INSTALLED)) {
	   ...
	   foreach($this->modules as $value) {
		if($value!='ot_gv.php'){ // as per E-204
	   ...
	   foreach($this->modules as $value) {
          $class = substr($value, 0, strrpos($value, '.'));
		   ...
		   // manual price adjustment
			if($class=='ot_total'){
				if($FSESSION->manual_option=='Y'){
					if($FSESSION->sign=='-')
						$rest_mp = substr($FSESSION->modify_price_prefix, 1);
					else
						$rest_mp= $FSESSION->modify_price_prefix;
					
					$output_string .= '<tr><td class="main" align="right">'. $FSESSION->purpose .'('.$FSESSION->sign.'):' .'</td><td align="right" class="main">'. $currencies->format($rest_mp).'</td></tr>';
				}
			}
          if ($GLOBALS[$class]->enabled) {
...
//
    function credit_selection() {
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
        ...
        $close_string  .= '<tr><td width="100%">' .  tep_draw_separator('pixel_trans.gif', '100%', '1') . '</td></tr>';
        reset($this->modules);
        $output_string = '';
        //while (list(, $value) = each($this->modules)) {
	   //FOREACH
            ...
            if ( ($use_credit_string !='' ) || ($selection_string != '') ) {
             $output_string .=  '<tr colspan="4"><td colspan="4" width="100%">' .  tep_draw_separator('pixel_trans.gif', '100%', '10') . '</td></tr>';
              $output_string = ' <tr class="moduleRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" >' . "\n" .
                               ...
                               '     <td class="main"><b>' . ((($GLOBALS[$class]->header) == "Gift Vouchers/Discount Coupons")? "Discount Coupon/Gift Voucher Redeem" :"") . '</b></td>' . $use_credit_string;
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
	   ...
	   foreach($this->modules as $value) {
          ...
          if ( ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) ) {
			...
			$pp=$FREQUEST->postvalue($post_var);
            if ($pp!='') $FSESSION->set($post_var,$pp);
...
//
    function pre_confirmation_check() {
      global $payment, $order, $credit_covers,$FSESSION;	  	  
      if (MODULE_ORDER_TOTAL_INSTALLED) {
        $total_deductions  = 0;
        reset($this->modules);		
	   ...
	   foreach($this->modules as $value) {
		...
		// this is for while editing the order the nettot is assigned for status1
		$order_id=$FSESSION->order_id;
		$ostate=$FSESSION->order_status;
		if($order_id > 0 ){
		if($ostate=='1') 
		$ord_tot=$order->info['nettotal'];
		else if($ostate=='2') {
		$ord_tot=$order->info['total'];
		}
		}else{
		$ord_tot=$order->info['total'];
		}
        if ($ord_tot - $total_deductions <= 0 ) { 
          $credit_covers = true; 
        }
      }
    }
    ...
    function clear_posts() {
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
	   ...
	   foreach($this->modules as $value) {
          $class = substr($value, 0, strrpos($value, '.'));
          if ( ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) ) {
		    $ff='c_'.$GLOBALS[$class]->code;
            $FSESSION->set('post_var',$ff);
