 ...
 */
	...
	function add_smtp_attachment($file, $name = '') {
      $this->smtp_attachments[] = array('file' => $file,
                                        'name' => $name);
    }
 ...
 */
	...
	// ============================================================
		...
		if(is_array($to_addr) && sizeof($to_addr)>0) {
              $to_address='';
              for($i=0;$i<sizeof($to_addr);$i++) {
                  $to_address.='<' . $to_addr[$i] . '>' . ',';
              }
              if($to_address!='') $to_address=substr($to_address,0,-1);
              $to =  $to_address;
              $to_addr=$to_address;
        }
        else 
...
if (EMAIL_SMTP_TRANSPORT=='smtp'){//start new code

require_once (DIR_FS_CATALOG.'includes/classes/mail/src/Exception.php');
require_once (DIR_FS_CATALOG.'includes/classes/mail/src/PHPMailer.php');
require_once (DIR_FS_CATALOG.'includes/classes/mail/src/SMTP.php');
...
$mail->Port = EMAIL_SMTP_PORT_SERVER;
if(EMAIL_SMTP_PORT_SERVER==587){
//Set the encryption system to use - ssl (deprecated) or tls
$mail->SMTPSecure = 'tls';}else{
$mail->SMTPSecure = 'ssl';}
...
//Read an HTML message body from an external file, convert referenced images to embedded,
		...
		if ($this->html && $this->html!=''){
          ...
          for ($i=0; $i<count($this->smtp_attachments); $i++) {
            $mail->addAttachment($this->smtp_attachments[$i]['file'], $this->smtp_attachments[$i]['name']);
			}
...
if (!$mail->send()) {
    $eresult= $mail->ErrorInfo;
} else {
    $eresult='Message sent';
}
	  return $eresult;
	 if ($wait_time==0) 
	    return false;
	  ...
	  return $eresult;
    // wait for seconds in Mail send
	if($wait_time!=0)
	{
	    usleep($wait_time * 7000000);
	}
}
