...
*/
  ...
  class manualOrder {
    ...
    function query($order_id) {
      ...
      $index = 0;
      $orders_products_query = tep_db_query("select orders_products_id, products_id, discount_id, products_quantity, discount_text, products_name, products_model,products_sku, products_price, products_tax, products_quantity, final_price, categories_name, concert_venue, concert_date, concert_time from " . TABLE_ORDERS_PRODUCTS . " where orders_id = '" . (int)$order_id . "' order by orders_products_id");
	
      while ($orders_products = tep_db_fetch_array($orders_products_query)) {
	  
	  		  	$discount_whole_text='....';
	  		if ($orders_products["discount_id"]>0 && $orders_products["discount_text"]!=''){
	  			$discount_whole_text=TEXT_DISCOUNT_APPLIED . '&nbsp;' . $orders_products["discount_text"] 
				.'<br>' . TEXT_ORIGINAL_AMOUNT . '&nbsp;' 
				. $currencies->format(tep_add_tax($orders_products['products_price'],$orders_products['products_tax'])*$orders_products['products_quantity']) 
				. '<br>' . TEXT_DISCOUNT . '&nbsp;' 
				.$currencies->format(tep_add_tax($orders_products['products_price']-$orders_products['final_price'],$orders_products['products_tax'])*$orders_products['products_quantity']) 
				. '<br>';
			}
