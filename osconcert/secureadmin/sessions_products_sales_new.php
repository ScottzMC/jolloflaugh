...
//  echo $FSESSION->login_groups_type;
  ...
  //lang
	define(TEXT_TICKETS_SOLD,'Tickets Sold (not Penguins) : ');
	...
	//concert_date
	$eventdate=isset($input_params['sel_eventdate'])?$input_params['sel_eventdate']:"All";
	// event time
...
//	if ($staff=="All") $display_header.=TEXT_CALL_CENTRE_STAFF . ":&nbsp;&nbsp;" . TEXT_ALL_STAFFS."\t";
	...
	if ($eventdate=="All") $display_header.=TEXT_DATE . ":&nbsp;&nbsp;" . TEXT_ALL_DATE."\t";
	if ($eventtime=="All") $display_header.=TEXT_EVENTTIME . ":&nbsp;&nbsp;" . TEXT_ALL_TIME."\t";
	...
	$sql="SELECT distinct(op.concert_date) from " . TABLE_ORDERS_PRODUCTS . " op WHERE op.concert_date !='' order by op.orders_id desc LIMIT 500";
	//echo $sql;
	$sql_result=tep_db_query($sql);
	$eventdate_array[]=array("id"=>"All","text"=>ALL_SEATS);
	while($row=tep_db_fetch_array($sql_result)){
		$eventdate_array[]=array("id"=>$row["concert_date"],"text"=>$row["concert_date"]);
		if ($row["concert_date"]==$eventdate){
			$display_header.=TEXT_DATE  . ":&nbsp;&nbsp;" . $row["concert_date"] . "&nbsp;&nbsp;";
		}
	}
	tep_db_free_result($sql_result);
	#######################################################################################
	...
	$sql="SELECT distinct(op.concert_time) from " . TABLE_ORDERS_PRODUCTS . " op WHERE op.concert_time !='' order by op.concert_time desc LIMIT 125";
	//echo $sql;
	$sql_result=tep_db_query($sql);
...
//	echo $sql;
	...
	$found_results=false;
		...
		$num_rows=0;
		$cur_row=0;
		$rows_each=5000;//REPORT_MAX_ROWS_PAGE;
		...
		if($_POST['sel_eventdate'] != "All")
		{
			
			$eventd.= " and op.concert_date ='".tep_db_prepare_input($_POST['sel_eventdate'])."' ";
			}
			
		if($_POST['sel_eventtime'] != "All")
		{
			$eventt.= " and op.concert_time ='".tep_db_prepare_input($_POST['sel_eventtime'])."' ";
			}
			...
			o.orders_status from " . TABLE_ORDERS_PRODUCTS . " op, " . TABLE_ORDERS . " o where op.orders_id=o.orders_id and date_format(o.date_purchased,'%Y-%m-%d')>='".tep_db_input($date_begin)."' and date_format(o.date_purchased,'%Y-%m-%d')<='".tep_db_input($date_end)."' ".$eventd." ".$eventt." ".$stats." order by ".$order_by." ";
	...
	$family_sum_sql="SELECT sum(op.products_quantity) as family_quantity from orders_products op WHERE op.events_type='F' ".$eventd." ".$eventt." ".$stats." ";
	$family_sum_query=tep_db_query($family_sum_sql);

	while($row3=tep_db_fetch_array($family_sum_query))
	{
	$family_sold=$row3['family_quantity']*4;
	}
	...
	//$ticket_sum_sql="SELECT sum(op.products_quantity) as tickets_sold from " . TABLE_ORDERS_PRODUCTS . " op
	// where events_type='G' and op.orders_products_status='3' ".$eventd." ".$eventt." ".$stats."  and op.events_type != 'X' and op.products_sku !=6";
	
	$ticket_sum_sql="SELECT sum(op.products_quantity) as tickets_sold from " . TABLE_ORDERS_PRODUCTS . " op, " . TABLE_ORDERS . " o where  op.orders_id=o.orders_id and orders_status!='0' and date_format(o.date_purchased,'%Y-%m-%d')>='".tep_db_input($date_begin)."' and date_format(o.date_purchased,'%Y-%m-%d')<='".tep_db_input($date_end)."' and events_type='G' ".$eventd." ".$eventt." ".$stats."  and op.events_type != 'X' and op.products_sku !=6";
	...
	//	$sum_sql='';
		...
		$tickets_sold=$row2['tickets_sold']+$family_sold;
      ...
      //echo "&nbsp;". ucfirst($selected_category) ."&nbsp;";
        switch(strtolower($selected_category))
		{
            ...
            //cartzone
                ...
                echo '&nbsp;&nbsp;' . TEXT_DATE . ': '.tep_draw_pull_down_menu("sel_eventdate",$eventdate_array,$eventdate);
				echo '&nbsp;&nbsp;' . TEXT_TIME . ': '.tep_draw_pull_down_menu("sel_eventtime",$eventtime_array,$eventtime);
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="2"><!-- count listings table //-->
			<tr>
				<td width="80%" align="left" class="smallText">
					PAGE: <?php if($query_split) echo $query_split->display_links($query_split_numrows, REPORT_MAX_ROWS_PAGE, REPORT_MAX_LINKS_PAGE, $page,tep_get_report_params()); ?>&nbsp;  <?php echo TOTAL_COUNT;?>: <b><?php echo $total_record_in_db ;?></b>	&nbsp;  <?php echo TEXT_TICKETS_SOLD;?><b><?php echo $tickets_sold ;?></b>			</td>
...
//		}
	...
	{
		global $coupon_total,$output_array,$all_totals,$tickets_sold,$found_results,$login_id,$report_filename,$selected_category,$currencies;
		$result="";
		if ($found_results){
		$report_display_header=strtolower($selected_category);
		...
		for ($icnt=0;$icnt<sizeof($output_array);$icnt++) {
			...
			if ($type=='head')
			{
				if($report_display_header==strtolower($element['content'])){
					...
					$result.="" . TEXT_SEATS . "\n";
					$result.=TEXT_ID . "," . SEAT_NAME . "," . ORDER_ID . "," . TICKET_ID . "," . TICKET_PRINTED . "," . DATE_ID . "," . DAY . "," . SHOW . "," . TEXT_CLIENT . "," . TEXT_BILLING_NAME . "," . QTY . "," . TEXT_STATUS . "," . TEXT_PAYMENT . "," . TEXT_PURCHASED . "," . TEXT_AMOUNT . "," . TEXT_TAX . "\n";
				} else{
					$report_display='reject';
				}
			} else if (($type==$sub) && ($report_display=='accept')){//
...
########################################################################################################				
			...
			foreach($list_tickets as $key=> $value)
			{
			$result.=$element["col2"] . "," . utf8_decode($element["col"]) . "," . utf8_decode($element["col1"]) . "," . $value . "," . $element["col10"] . "," . $element["col4"] . "," . utf8_decode($element["col11"]) . "," . utf8_decode($element["col3"]) . "," . utf8_decode($element["col8"]) . "," .  "1," . utf8_decode($element["col5"]) . "," . utf8_decode($element["col12"]) . "," . $element["col13"] . "," .  $currencies->format($element["col100"]/ sizeof($list_tickets)) . "," . $element["col98"] . "," . $element["col20"] . "\n";
			}
			...
			// and process as normal
			$result.=
			$element["col2"] . "," . utf8_decode($element["col"]) . "," . utf8_decode($element["col1"]) . "," . $element["col9"] . "," . $element["col10"] . "," . $element["col4"] . "," . utf8_decode($element["col11"]) . "," . utf8_decode($element["col3"]) . "," . utf8_decode($element["col8"]) . "," . $element["col7"] . "," . utf8_decode($element["col5"]) . "," . utf8_decode($element["col12"]) . "," . $element["col13"] . "," . $element["col6"] . "," . $element["col98"] . "," . $element["col20"] . "\n";
			}
			}else
			{			

			$result.=$element["col2"] . "," . utf8_decode($element["col"]) . "," . utf8_decode($element["col1"]) . "," . $element["col9"] . "," . $element["col10"] . "," . $element["col4"] . "," . utf8_decode($element["col11"]) . "," . utf8_decode($element["col3"]) . "," . utf8_decode($element["col8"]) . "," . $element["col7"] . "," . utf8_decode($element["col5"]) . "," . utf8_decode($element["col12"]) . "," . $element["col13"] . "," . $element["col6"] . "," . $element["col98"] . "," . $element["col20"] . "\n";
			}
			...
			//	if(tep_get_rounded_amount($coupon_total)>0)
				...
				if(tep_get_rounded_amount($net_total)>0){
				$result.=",," . TEXT_TICKETS_SOLD . ",,,,,,,\"" . $tickets_sold. "\"\n";
				}
				//Graeme Tyson, sakwoya@sakwoya.co.uk,  March  2012 for osConcert	
