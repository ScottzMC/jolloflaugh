...
//Page header
 function RoundedRect($x, $y, $w, $h,$r, $style = '')
	...
	$date, $image_function, $customer_id, $web_address,$sna_name,$vat_shift_y;
  ...
  //  $this->Ln(0);
	...
	$sna_name = tep_html_entity_decode(STORE_NAME_ADDRESS);

		$sna_name = iconv('UTF-8', 'ISO_8859-9', $sna_name);	
			
	$this->MultiCell(90, 4, tep_html_entity_decode($sna_name),0,'L');
	$pos_y += ((substr_count($sna_name, "\n")+1) * 4.4);
	...
	$this->Text(9, $pos_y+15 ,tep_html_entity_decode(PRINT_INVOICE_TITLE) . "#" . (int)$FREQUEST->getvalue('oID') );
...
if(findextension(PDF_INVOICE_IMAGE) == 'ai' || findextension(PDF_INVOICE_IMAGE) == 'eps'){
	...
	$cust=tep_html_entity_decode(tep_address_format($order->customer['format_id'], $order->customer, '', '', "\n"));
	$cust = iconv('UTF-8', 'ISO_8859-9', $cust);
	
	$pdf->MultiCell(73, 4, $cust,0,'L');
