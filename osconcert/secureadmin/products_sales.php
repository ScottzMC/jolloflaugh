...
//  define(TEXT_QTY,'Qty');
define(TEXT_TICKETS_SOLD,'Products Sold (not Season Tickets) : ');
	define(RESERVED_SEATING,'Products');
...
//  define(ADMIN_USER,'admin user');
  ...
  //Graeme Tyson, sakwoya@sakwoya.co.uk,  March  2012 for osConcert
	...
	//$date_purchased=isset($input_params['sel_date_purchased'])?$input_params['sel_date_purchased']:"All";
	//Users Admin or web
	//$users=isset($input_params['sel_user'])?$input_params['sel_user']:"All";
	//call centre staff
	$staff=isset($input_params['sel_staff'])?$input_params['sel_staff']:"All";
	$products=isset($input_params['sel_products'])?$input_params['sel_products']:"All";
	// payment statuses
	...
	if ($date_begin==""){
		$sql =  "select date_sub('".getServerDate()."', interval 1 day) begin,'".date('Y-m-d')."' as end";
	...
	if ($products=="All") $display_header.=TEXT_PRODUCTS . ":&nbsp;&nbsp;" . TEXT_ALL_PRODUCTS."\t";
	...
	####################
	//Box Office
		$sql ="select concat(c.customers_firstname,' ',c.customers_lastname) as customers_name from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . 
					 " a on c.customers_default_address_id = a.address_book_id where a.customers_id = c.customers_id and entry_country_id='999' order by customers_name";
//	echo $sql;
	$sql_result=tep_db_query($sql);
...
$sql = "select distinct products_name,events_type from " . TABLE_ORDERS_PRODUCTS . "  where orders_products_status!='0' AND events_type !='P'";
	$sql_result=tep_db_query($sql);
	$products_array[]=array("id"=>"All","text"=>TEXT_ALL_PRODUCTS);
	while($row=tep_db_fetch_array($sql_result)){
		if($row["events_type"]=='P'){
			$product_name=RESERVED_SEATING;
		}else{
			$product_name=$row["products_name"];
		}
		$products_array[]=array("id"=>$product_name,"text"=>$product_name);
		if ($product_name==$products){
			$display_header.=TEXT_PRODUCTS  . ":&nbsp;&nbsp;" . $product_name . "&nbsp;&nbsp;";
		}
	}
	...
	$found_results=false;
		...
		if ($product_show) {
			if($products!="All") {
				$where.= " and products_name='".tep_db_prepare_input($products)."' ";
			}
		$stats=" ";
		if ($product_show) {
			if($order_status!="All") {
		...
		if(CUST_LIST=='true'){
			...
			$sql = "select (op.final_price*op.products_quantity) as final_price,op.products_tax, o.customers_id,o.orders_id,o.ticket_printed,op.support_packs_type, op.orders_products_id,op.events_type,op.events_id,op.products_type,op.categories_name,op.concert_venue,op.concert_date,op.concert_time,o.customers_name as order_name, concat(LTRIM(c.customers_firstname),' ',LTRIM(c.customers_lastname)) as customers_name,op.products_id as item_id,o.customers_postcode,o.customers_email_address,op.products_quantity,op.orders_products_status,o.payment_method,o.billing_name,op.products_model,op.products_name as item_name, o.orders_status from " . TABLE_ORDERS_PRODUCTS . " op, " . TABLE_ORDERS . " o, " . TABLE_CUSTOMERS . " c where o.customers_id =c.customers_id and op.orders_id=o.orders_id and orders_status!='0' and date_format(o.date_purchased,'%Y-%m-%d')>='".tep_db_input($date_begin)."' and date_format(o.date_purchased,'%Y-%m-%d')<='".tep_db_input($date_end)."' ".$where." ".$event." ".$stats." order by ".$order_by." ";
...
// so we need to aggregate the amounts here
			...
			//	$sum_sql='';
				...
				while($row1=tep_db_fetch_array($sum_query)){
			    	$new_tot_amount += tep_add_tax($row1['final_price'],$row1["products_tax"]);
...
//we now have tot_amount to go forward with
		...
		for ($icnt=0;$icnt<sizeof($db_result);$icnt++) {
			...
			while($row = tep_db_fetch_array($sql_result)) {
				...
				$item=$row["item_id"];
			$test=$order_status_array[$row["orders_status"]];
			...
			if($season_ticket==1)
			{
				...
				if($row["support_packs_type"]=="G"){
					...
					//$ticket_id.= $row["orders_id"] .'-'.$row['customers_id'] .'-'.$item.$running_number.'<br/>';
					}
				}else{
				$ticket_id= $row["orders_id"] .'_'.$item;
				//$ticket_id= $row["orders_id"] .'-'.$row['customers_id'] .'-'.$item;
				}
				//$ticket_id=$item;
				...
				$sum_sql="SELECT sum(op.final_price*op.products_quantity) as final_price,op.products_tax from " . 
										TABLE_ORDERS_PRODUCTS . " op where op.orders_id='" . $order_id . "'" . $prd_type_where . " group by orders_id,op.final_price,op.products_quantity,op.products_tax";
			...
			//	$sum_sql='';
				...
				if (!$summary_show) {
					...
					'col4'=>$date_id,
					'col11'=>$showname,
					//'col11'=>$showdate.' '. $showname. ' '.$showtime,
					'col2'=>$i,
					'col9'=>$ticket_id,
					'col10'=>$ticket_printed,
					'col3'=>$row['order_name'],
					'col20'=>$row['customers_email_address'],
					'col21'=>$row['customers_postcode'],
					'col8'=>$row['billing_name'],
					//'col8'=>$session_time,
					//'col7'=>$row['products_quantity'],
					'col7'=>$quantity,
					'col5'=>$test,
					'col12'=>$payment,
...
<tr> 
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="2"><!-- main table //-->
        <tr>
		  	<td>
			<?php 	echo tep_draw_form("f",FILENAME_PRODUCTS_SALES);
					$report_display='';
      ...
      //echo "&nbsp;". ucfirst($selected_category) ."&nbsp;";
        switch(strtolower($selected_category)){
            ...
            //cartzone
				...
				echo '&nbsp;&nbsp;' . TEXT_PRODUCTS_SOLD . '&nbsp;' . tep_draw_pull_down_menu("sel_products",$products_array,$products);
                ...
                <tr>
					...
					<td id="event_2"></td>
					<td class="main" valign="middle" align="right" >
						<?php echo '<a href="javascript:doReport(4);">' . tep_image_button('button-export-special.gif', IMG_EXPORT_SPECIAL) . '</a>';?></td>
				</tr>
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="2"><!-- count listings table //-->
			<tr>
				<td width="80%" align="left" class="smallText">
					PAGE: <?php if($query_split) echo $query_split->display_links($query_split_numrows, REPORT_MAX_ROWS_PAGE, REPORT_MAX_LINKS_PAGE, $page,tep_get_report_params()); ?>&nbsp;  <?php echo TOTAL_COUNT;?>: <b><?php echo $total_record_in_db ;?></b>				</td>
...
?>
			...
			<?php		} else if ($type=='startdiv') { ?>
                       ...
                       #1 add the field name to the SQL (line 189)
						...
						<td class='dataTableHeadingTitleContent' width="100"><?php echo TEXT_CUSTOMER;?></td>	
...
<!--					  <td class='dataTableContent' style="cursor:hand" align="center"><a href="mailto:<?php //echo $element['col4'];?>"><img src="images/icons/mail.gif" border="0"></a></td>-->
		...
		for ($icnt=0;$icnt<sizeof($output_array);$icnt++) {
			...
			//Graeme Tyson, sakwoya@sakwoya.co.uk,  March  2012 for osConcert	
				...
				if($element["col99"]=="G"){//GA Ticket
				  ...
				  foreach($list_tickets as $key=> $value){	unset($cols);
				  //unable to handle 
...
//		}
	...
	function generate_excel(){
		global $coupon_total,$output_array,$all_totals,$found_results,$login_id,$report_filename,$selected_category,$currencies;
		$result="";
		if ($found_results){
		$report_display_header=strtolower($selected_category);
		...
		for ($icnt=0;$icnt<sizeof($output_array);$icnt++) {
			...
			if ($type=='head'){
				if($report_display_header==strtolower($element['content'])){
					...
					$result.="" . TEXT_SEATS . "\n";
					$result.=TEXT_ID . "," . SEAT_NAME . "," . ORDER_ID . "," . TICKET_ID . "," . TICKET_PRINTED . "," . DATE_ID . "," . SHOW . "," . TEXT_CUSTOMER . "," . TEXT_BILLING_NAME . "," . QTY . "," . TEXT_STATUS . "," . TEXT_PAYMENT . "," . TEXT_AMOUNT . "\n";
				} else{
					$report_display='reject';
				}
			} else if (($type==$sub) && ($report_display=='accept')){//
			...
			///////////////////////////////////////////////////////////////////////////////////////////
				...
				//Code change RM 
				//$result.=SEAT . "," . ORDER_ID . "," . TICKET_ID . "," . DATE_ID . "," . TEXT_CUSTOMER . "," . TEXT_BILLING_NAME . "," . QTY . "," . TEXT_STATUS . "," . TEXT_AMOUNT . "\n";
			//} else if (($type=='item') && ($report_display=='accept')){
				//$result.=$element["content"] . "\n";
			} else if (($type=='row') && ($report_display=='accept')){
				...
				//col20 email address
#################################################################################################################################################
//ADD AN ELEMENT TO THE END OF THESE RESULTS TO QUICKLY LIST ASSOCIATED DATA IN THE CSV DOWNLOAD: EMAIL ADDRESS APPENDS HERE AS AN EXAMLE
#################################################################################################################################################
...
########################################################################################################				
				if($element["col99"]=="G"){//GA Ticket
				  ...
				  foreach($list_tickets as $key=> $value){
					  ...
					  "\"".$element["col4"] . "\"," .
					  "\"".$element["col11"] . "\"," .
					  "\"". utf8_decode($element["col3"]) . "\"," .
					  "\"".utf8_decode($element["col8"]) . "\"," .
					  "\"1\"," . 
					  "\"".$element["col5"] . "\"," .
					  "\"".$element["col12"] . "\"," .
					  "\"".$currencies->format($element["col100"]/ sizeof($list_tickets)) . "\"," .
					  "\"".$element["col21"] . "\"," .
					  "\"".$element["col20"] . "\"\n";
				}
				...
				//$result.="\"".$element["col2"] . "\"," . "\"".$element["col"] . "\"," . 
//				         "\"".$element["col1"] . "\"," .
//				          $value . "," . 
//				         "\"".$element["col10"] . "\"," .
//				         "\"".$element["col4"] . "\"," .
		...
		for ($icnt=0;$icnt<sizeof($output_array);$icnt++) {
			...
			//} else if (($type=='item') && ($report_display=='accept')){
				...
				if($element["col99"]=="G"){//GA Ticket
				  ...
				  if(sizeof($list_tickets)>1){
				  foreach($list_tickets as $key=> $value){
				 $result.=$element["col2"] . "," . $element["col"] . "," . $element["col1"] . "," . $value . "," . $element["col10"] . "," . $element["col4"] . "," . $element["col11"] . "," . utf8_decode($element["col3"]) . "," . utf8_decode($element["col8"]) . "," .  "1," . $element["col5"] . "," . $element["col12"] . ",\"" .  $currencies->format($element["col100"]/ sizeof($list_tickets)) . "\"," . $element["col20"] . "\n";
				}
				  
				  }else{//just one ticket strip out the only BR				  
					$element["col9"]=$list_tickets[0];
					// and process as normal
					$result.=$element["col2"] . "," . $element["col"] . "," . $element["col1"] . "," . $element["col9"] . "," . $element["col10"] . "," . $element["col4"] . "," . $element["col11"] . "," . utf8_decode($element["col3"]) . "," . utf8_decode($element["col8"]) . "," . $element["col7"] . "," . $element["col5"] . "," . $element["col12"] . ",\"" . $element["col6"] . "\"," . $element["col20"] . "\n";
					}
					}else{			
				
				$result.=$element["col9"] . "," . $element["col13"] . "\n";
				}
